import { IRange2, Range2, IRect, Rect, IVec2, Vec2, IPool, IPoolable } from "goodcore";
import { Observable } from "rxjs/Observable";
import { BehaviorSubject } from "rxjs/BehaviorSubject";
import { Subscription } from "rxjs/Subscription";
import { IInitable } from "goodcore";
...
declare namespace goodstore {
    ...
    export class DataStoreConsumable implements IInitable<DataStoreConsumable> {
        init(obj: Partial<DataStoreConsumable>): DataStoreConsumable;
        totalCells: Vec2;
        totalPx: Vec2;
        loadPort: Range2;
        pxScope: Rect;
    ...
    export class DataStoreRequestDto implements IInitable<DataStoreRequestDto> {
        init(obj: Partial<DataStoreRequestDto>): DataStoreRequestDto;
        sourceId: string;
        requestId: number;
        cellRequest: IRange2;
    }
    ...
    export class RequestConfig implements IRequestConfig, IInitable<RequestConfig> {
        init(obj: Partial<RequestConfig>): RequestConfig;
        viewPort: Range2;
        requestMargin: Vec2;
        pxViewPort: Range2;
        pxRequestMargin: Vec2;
    ...
    export class DataRow implements IDataRowDto, IPoolable, IInitable<DataRow> {
        init(obj: Partial<DataRow>): DataRow;
