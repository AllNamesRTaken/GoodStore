...
describe("DataStore", function () {
    ...
    it("should load make correct number of calls and retain the correct number of pages", function () {
        ...
        let handle = ds.register(new RequestConfig());
        let fetchCounter = 0;
        Util.proxyFn(ds, "fetchData", function (org, ...args) {
            fetchCounter += 1;
            return org(...args);
        });
        let viewPort = new Range2(0, 0, 4, 2);
    ...
    it("should not evict currently viewed pages in multi caller setting", function () {
        ...
        let dsc = {
            ...
            retainSize: 2,
            endPointFn: (payload) => {
                let result = new Rx.Observable(function (sub) {
    ...
    it("should cancel ongoing call when new call commes before old returns", function (done) {
        ...
        let dsc = {
            ...
            retainSize: 6,
            endPointFn: (payload) => {
                let result = Rx.Observable.create((sub) => {
                    setTimeout(() => {
                        if (!sub.closed) {
                            sub.next(mocDataResult.shift());
                            sub.complete();
    ...
    it("should return 10*10 pages with a total of 100*100 cells in less than 10ms", function () {
        ...
        let dsc = {
            ...
            retainSize: 100,
            endPointFn: (payload) => {
                let result = new Rx.Observable((sub) => {
                    sub.next(mocDataResult.shift());
                });
                return result;
            }
