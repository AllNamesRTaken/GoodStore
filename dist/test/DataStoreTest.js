...
describe("DataStore", function () {
    it("should load correct data when cell ports are used", function (done) {
        var mocDataResult = Arr.shallowCopy(mocDataResult1);
        var dsc = {
        ...
        var ds = new DataStore(dsc);
        var handle = ds.register(new RequestConfig());
        var req = 0;
        ...
        var viewPort = new Range2(0, 0, 4, 2);
    ...
    it("should load correct data when using pxViewPort", function (done) {
        ...
        var pxViewPort = new Range2(0, 0, 40, 20);
    ...
    it("should load make correct number of calls and retain the correct number of pages", function () {
        ...
        var handle = ds.register(new RequestConfig());
        var fetchCounter = 0;
        Util.proxyFn(ds, "fetchData", function (org) {
            var args = [];
    ...
    it("should return the correct loadPort to a given retainMargin and corner cases", function (done) {
        var mocDataResult = Arr.shallowCopy(mocDataResult2);
        var dsc = {
            pageSize: new Vec2(2, 2),
            retainSize: 4,
            endPointFn: function (payload) {
        ...
        var ds = new DataStore(dsc);
        var handle = ds.register(new RequestConfig().init({ requestMargin: new Vec2(1, 1) }));
        var req = 0;
        handle.stream.subscribe({
            next: function (v) {
                switch (req) {
        ...
        var viewPort = new Range2(0, 0, 2, 2);
        handle.load({ viewPort: viewPort });
        viewPort.pos.x = 1;
        viewPort.pos.y = 1;
        handle.load({ viewPort: viewPort });
    ...
    it("should not evict currently viewed pages in multi caller setting", function () {
        ...
        var ds = new DataStore(dsc);
        var handle1 = ds.register(new RequestConfig().init({ requestMargin: new Vec2(0, 0) }));
        var handle2 = ds.register(new RequestConfig().init({ requestMargin: new Vec2(0, 0) }));
        var viewPort1 = new Range2(0, 0, 4, 2);
        handle1.load({ viewPort: viewPort1 });
        var viewPort2 = new Range2(0, 2, 4, 2);
    ...
    it("should return 10*10 pages with a total of 100*100 cells in less than 10ms", function () {
        var mocDataResult = Arr.shallowCopy(mocDataResult3);
        var canceled = 0;
        var dsc = {
        ...
        var ds = new DataStore(dsc);
        var myStore = ds.register(new RequestConfig().init({ requestMargin: new Vec2(0, 0) }));
        var req = 0;
        ...
        var viewPort1 = new Range2(0, 0, 100, 100);
        var count = 10;
        Timer.start();
        Util.loop(count, function () { return myStore.load({ viewPort: viewPort1, forceStream: true }); });
        Timer.stop();
